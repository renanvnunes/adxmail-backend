...
router.get('/', cache.set('memory', 3600, 'emails'), verifyJWT, verifyEmpresa, (req, res, next) => {
	...
	await emailsSchema.find(query).then(async resp => {
...
router.put('/', verifyJWT, verifyEmpresa, (req, res, next) => {
	...
	await emailsSchema.updateOne({'_id' : mongoose.Types.ObjectId(id)}, {'$set' : new_data}, {'upsert' : true}).then(resp => {
