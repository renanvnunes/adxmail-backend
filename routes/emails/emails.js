...
router.get(
	...
	async (req, res) => {
		...
		const resp = await emailsSchema.findOne({
...
router.post(
	...
	async (req, res) => {
		...
		if (imagem_topo_adicional != undefined && imagem_topo_adicional != null) {
			const split_img_topo = imagem_topo_adicional.split('/');
			const type_img_topo = split_img_topo[1].split(';')[0];

			const data = imagem_topo_adicional.replace(`data:image/${type_img_topo};base64,`, '');
			const buffer = Buffer.from(data, 'base64');

			image_topo_name = `email_${Math.random()}.${type_img_topo}`;
			const key_foto_topo = `${process.env.DO_SP_FOLDER}/company/${empresa_id}/emails/${image_topo_name}`;

			if (type_img_topo === 'gif') {
				await writeFile(image_dir + image_topo_name, buffer);

				fs.readFile(image_dir + image_topo_name, async (error, file) => {
					const bucketParams = {
						Bucket: process.env.DO_SP_NAME,
						ContentType: `image/${type_img_topo}`,
						Key: key_foto_topo,
						ACL: 'public-read',
						Body: file
					};
					...
					if (success) {
						await Jimp.read(
							success
								...
								.quality(70)
								.write(image_dir + image_topo_name)
						).then(async () => {
							fs.readFile(image_dir + image_topo_name, async (error, file) => {
								const bucketParams = {
									Bucket: process.env.DO_SP_NAME,
		...
		if (imagem_rodape_adicional != undefined && imagem_rodape_adicional != null) {
			const split_img_rodape = imagem_rodape_adicional.split('/');
			const type_img_rodape = split_img_rodape[1].split(';')[0];

			const data = imagem_rodape_adicional.replace(`data:image/${type_img_rodape};base64,`, '');
			const buffer = Buffer.from(data, 'base64');

			image_rodape_name = `email_${Math.random()}.${type_img_rodape}`;
			const key_foto_rodape = `${process.env.DO_SP_FOLDER}/company/${empresa_id}/emails/${image_rodape_name}`;

			if (type_img_rodape === 'gif') {
				await writeFile(image_dir + image_rodape_name, buffer);

				fs.readFile(image_dir + image_rodape_name, async (error, file) => {
					const bucketParams = {
						Bucket: process.env.DO_SP_NAME,
						ContentType: `image/${type_img_rodape}`,
						Key: key_foto_rodape,
						ACL: 'public-read',
						Body: file
					};
					...
					if (success) {
						await Jimp.read(
							success
								.resize(700, Jimp.AUTO)
								.quality(70)
								.write(image_dir + image_rodape_name)
						).then(async () => {
							fs.readFile(image_dir + image_rodape_name, async (error, file) => {
								const bucketParams = {
									Bucket: process.env.DO_SP_NAME,
		...
		if (req.body.produtos.length > 0) {
			...
			if (imagens_baixadas.length == 0) {
				res.send({
					success: 0,
					message: 'Erro ao baixar as imagens dos produtos, tente novamente.'
				});
				return;
			}
