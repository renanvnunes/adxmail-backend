...
// WhatsApp submenu
import contacts from './routes/contacts/contacts.js'
import segmentations from './routes/segmentations/segmentations.js'
...
app.get('/', function (req, res) {
	res.send({Server: 'Api Express AdxMail'})
});  
...
app.use('/segmentations', segmentations)
...
app.listen(type_server == 'local' ? 3000 : process.env.PORT, () => {
	console.log(`Express Server rodando em http://localhost:${type_server == 'local' ? 3000 : process.env.PORT}`)
})